...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === "view") {
      ...
      if (!preset) {
        await interaction.reply({ content: `❌ Unknown preset \`${id}\`.`, flags: 1 << 6 });
        return;
      }
      ...
      if (c.infractionsConfig) {
        fields.push({
          ...
          value: [
            ...
            `Auto-demote at 3 strikes: ${c.infractionsConfig.autoDemotionEnabled ? "✅" : "❌"}`,
            `DM on infraction: ${c.infractionsConfig.dmOnInfraction ? "✅" : "❌"}`,
      ...
      if (c.moderationConfig) {
        fields.push({ name: "Moderation", value: `DM on action: ${c.moderationConfig.dmOnAction ? "✅" : "❌"}`, inline: true });
      }

      fields.push({
        name: "⚠️ What stays unchanged",
    ...
    if (sub === "apply") {
      if (!(await isManager(interaction))) {
        await interaction.reply({ content: "❌ Only server managers can apply presets.", flags: 1 << 6 });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        .setTitle(`✅ Preset Applied: ${preset.name}`)
