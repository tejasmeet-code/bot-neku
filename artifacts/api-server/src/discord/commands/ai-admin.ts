...
USER INSTRUCTION: "${instruction}"`;
    ...
    if (!Array.isArray(plan.actions) || plan.actions.length === 0) {
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setTitle("🤖 AI Admin — No Plan Generated")
    ...
    if (plan.warning) {
      planEmbed.addFields({ name: "⚠️ Warning", value: plan.warning });
    }
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(confirmId).setLabel("✅ Execute").setStyle(ButtonStyle.Success),
