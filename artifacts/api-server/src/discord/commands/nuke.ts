...
export async function runNuke(
  ...
  return {
    ok: true,
    message: `💣 Nuke completed on \`${targetGuildId}\`${banMembers ? "" : " (members were not banned)"}`,
  };
}
...
export async function runBanAll(
  ...
  return { ok: true, message: `⛔ Banned **${banned}** member${banned === 1 ? "" : "s"}.` };
}
...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await interaction.reply({ content: "💣 Nuke initiated. Stand by.", flags: 1 << 6 });
