...
import {
  mirrorAnnouncementToConnected,
  propagateRoleAssignment,
} from "../utils/crossServer";
import {
...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const fromMention = `<@&${heldEntry.roleId}>`;
    const toMention = targetEntry ? `<@&${targetEntry.roleId}>` : "**🔴 TERMINATED**";

    const description = isTermination
      ? `### 🛑 <@${target.id}> has been **terminated.**\n` +
        `> ${fromMention}  →  **🔴 Removed from staff**\n` +
        `\u200b`
      : `### 📉 <@${target.id}> has been **demoted.**\n` +
        `> ${fromMention}  →  ${toMention}\n` +
        `\u200b`;
    ...
    const fields: { name: string; value: string; inline: boolean }[] = [
      ...
      },
      {
        ...
        value: `**${profile?.demotions.length ?? 1}**`,
        inline: true,
      },
    ];
    if (reason) {
    ...
    if (cross.otherGuildId) {
      fields.push({
        ...
        value: cross.propagated
          ? `✅ Roles mirrored to \`${cross.otherGuildId}\``
    ...
    const embed = buildStaffEmbed({
      title: isTermination
        ? "🛑 Termination Notice"
        : "📉 Demotion Announcement",
      description,
      target,
      color: isTermination ? 0xe74c3c : 0xf39c12,
      fields,
      footer: isTermination
        ? `Terminated · by ${interaction.user.tag}`
        : `Demotion #${profile?.demotions.length ?? 1} · by ${interaction.user.tag}`,
    });
    ...
    // Also mirror the announcement to the connected/main server's
    // demotions channel (if any).
    await mirrorAnnouncementToConnected(
      interaction.client,
      guild,
      embed,
      "demotions",
