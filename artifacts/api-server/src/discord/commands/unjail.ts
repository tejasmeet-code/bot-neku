...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (restored > 0)
      restoreNotes.push(`restored **${restored}** stashed role${restored === 1 ? "" : "s"}`);
    if (missing > 0)
      restoreNotes.push(`**${missing}** role${missing === 1 ? "" : "s"} no longer exist`);
    if (aboveBot > 0)
      restoreNotes.push(`**${aboveBot}** role${aboveBot === 1 ? "" : "s"} are above mine and were skipped`);

    const tail = restoreNotes.length > 0 ? ` (${restoreNotes.join(", ")})` : "";

    await interaction.editReply(
      `${CE.success.str} **${target.tag}** has been released from jail. Reason: ${reason}${tail}`,
    );
  },
};
