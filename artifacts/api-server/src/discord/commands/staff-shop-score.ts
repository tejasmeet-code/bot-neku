  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === "view") {
      ...
      const salesLines = stats.sales.slice(-10).reverse().map((s, i) =>
        `**${i + 1}.** ${s.item} @ ${s.price}${s.rating != null ? ` ⭐${s.rating}/10` : ""} — <t:${Math.floor(s.date / 1000)}:d> (Ticket: \`${s.ticketId}\`)`,
      );
      ...
      const embed = prettyEmbed({
        ...
        fields: [
          ...
          { name: "Avg Rating", value: avg != null ? `⭐ **${avg.toFixed(1)}/10**` : "*No ratings yet*", inline: true },
