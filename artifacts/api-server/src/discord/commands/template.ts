...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    if (!isDev(interaction.user.id)) {
      await interaction.reply({ content: "❌ Templates are restricted to bot developers.", flags: 1 << 6 });
      return;
    }
    ...
    if (sub === "save") {
      ...
      if (existing) {
        await interaction.reply({
          content: `❌ A template named **${name}** already exists (\`${existing.id}\`). Delete it first or choose a different name.`,
          flags: 1 << 6,
        });
        return;
      }
    ...
    if (sub === "list") {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`📋 Config Templates (${templates.length})`)
    ...
    if (sub === "view") {
      ...
      if (!tpl) {
        await interaction.reply({ content: `❌ No template found for \`${nameOrId}\`.`, flags: 1 << 6 });
        return;
      }
    ...
    if (sub === "apply") {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`✅ Template Applied: ${tpl.name}`)
        .setColor(COLORS.success)
        .setDescription(
          `Modules and per-module settings from **${tpl.name}** have been applied to this server.\n\n` +
          `⚠️ **Channels and roles were not changed** — update them via \`/config channel\` and \`/staff-role-add\`.`,
        )
