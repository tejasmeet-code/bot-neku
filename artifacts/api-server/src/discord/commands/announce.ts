...
const command: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => sub
      .setName("post")
      .setDescription("Post an announcement immediately — a popup will let you write multi-paragraph messages.")
    ...
    .addSubcommand(sub => sub
      .setName("schedule")
      .setDescription("Schedule an announcement — a popup will let you write multi-paragraph messages.")
      .addChannelOption(o => o.setName("channel").setDescription("Channel to post in").setRequired(true).addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
      .addStringOption(o => o.setName("title").setDescription("Announcement title").setRequired(true).setMaxLength(256))
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === "post") {
      ...
      const modal = new ModalBuilder()
        ...
        .setTitle("Write Announcement Message");
      modal.addComponents(
        new ActionRowBuilder<TextInputBuilder>().addComponents(
          new TextInputBuilder()
            .setCustomId("message")
            .setLabel("Message body (multi-paragraph supported)")
      ...
      try {
        await channel.send({
          ...
          embeds: [prettyEmbed({
            ...
            description: message,
            color,
            footer: `Announced by ${interaction.user.tag} • Zenvy`,
          })],
        });
      } catch {
        await submit.editReply({ embeds: [errorEmbed("Failed", "Could not post to that channel — check my permissions.")] });
        return;
      }

      await submit.editReply({ embeds: [prettyEmbed({
    ...
    if (sub === "schedule") {
      ...
      if (!scheduledFor) {
        await submit.editReply({ embeds: [errorEmbed(
          "Invalid date/time",
      ...
      await submit.editReply({ embeds: [prettyEmbed({
        ...
        color: COLORS.info,
        footer: "Use /announce cancel to remove it • Zenvy",
      })] });
      return;
    }
    ...
    if (sub === "list") {
      ...
      const lines = all.slice(0, 15).map(e => {
      ...
      await interaction.editReply({ embeds: [prettyEmbed({
        ...
        color: COLORS.info,
        footer: "Use /announce cancel <id> to remove one • Zenvy",
      })] });
      return;
    }
    ...
    if (sub === "cancel") {
      ...
      const deleted = await deleteScheduledAnnounce(interaction.guildId, id);
      if (!deleted) {
