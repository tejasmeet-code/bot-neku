...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const fromMention = heldEntry ? `<@&${heldEntry.roleId}>` : "_not yet on staff_";
    const toMention = `<@&${targetEntry.roleId}>`;

    const description =
      `### 🎉 <@${target.id}> has been **promoted!**\n` +
    ...
    const fields: { name: string; value: string; inline: boolean }[] = [
      ...
      {
        name: "🔻 From role",
        value: heldEntry ? `<@&${heldEntry.roleId}>` : "_not yet on staff_",
        inline: true,
      },
      {
        name: "🔺 New role",
        value: `<@&${targetEntry.roleId}>`,
        inline: true,
      },
      {
        name: "📈 Total promotions",
        value: `**${profile?.promotions.length ?? 1}**`,
        inline: true,
      },
    ];
    if (reason) {
    ...
    const embed = buildStaffEmbed({
      ...
      color: 0x2ecc71,
      fields,
      footer: `Promotion #${profile?.promotions.length ?? 1} · by ${interaction.user.tag}`,
    });
    ...
    // promotions channel (if any) so the same news appears there.
