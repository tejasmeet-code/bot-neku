...
function verdict(pct: number): string {
  if (pct >= 90) return "Soulmates. Ring it up. 💍";
  if (pct >= 75) return "Real chemistry here. ✨";
  if (pct >= 50) return "Could work with effort. 🙂";
  if (pct >= 25) return "Stay friends, please. 🫤";
  return "Disaster waiting to happen. 💀";
}
