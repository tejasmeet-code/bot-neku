...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    events.push({
      ...
      line: `🟢 First joined staff — ${fmtDate(profile.firstJoinedAt)}`,
    });

    for (const p of profile.promotions) {
      events.push({
        at: p.at,
        line: `📈 Promoted from **${describeRole(p.fromRoleId, roleNames)}** → **${describeRole(p.toRoleId, roleNames)}** by <@${p.byUserId}> ${p.reason ? `· _${p.reason}_` : ""}`,
      });
    }
    for (const d of profile.demotions) {
      ...
      events.push({
        at: d.at,
        line: `📉 Demoted from **${describeRole(d.fromRoleId, roleNames)}** → ${tgt} by <@${d.byUserId}> ${d.reason ? `· _${d.reason}_` : ""}`,
      });
    }
    if (profile.terminated && profile.terminatedAt) {
      events.push({
        at: profile.terminatedAt,
        line: `🔴 Terminated — ${fmtDate(profile.terminatedAt)}`,
      });
    }
