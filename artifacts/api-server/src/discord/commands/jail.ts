...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (target.id === interaction.user.id) { await interaction.reply({ content: "You can't jail yourself.", ephemeral: true }); return; }
    if (target.id === interaction.client.user.id) { await interaction.reply({ content: "I can't jail myself.", ephemeral: true }); return; }
    ...
    try {
      ...
      await interaction.editReply("Failed to apply the Jailed role.");
      return;
    }
    ...
    await sendPunishmentDM(target, { action: "jail", serverName: interaction.guild.name, reason, caseNumber: caseEntry.case_number, guildId: interaction.guildId, proof });

    const stripNote = removed > 0 ? ` Removed **${removed}** role${removed === 1 ? "" : "s"}.` : "";
    const warnNote = couldNotRemove > 0 ? ` ⚠️ ${couldNotRemove} role${couldNotRemove === 1 ? "" : "s"} couldn't be removed.` : "";

    await interaction.editReply({
      embeds: [successEmbed(`Jailed — Case #${caseEntry.case_number}`, `**${target.tag}** has been jailed.\n**Reason:** ${reason}.${stripNote}${warnNote}`)],
    });
  },
};
