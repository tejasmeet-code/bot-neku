...
const command: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const [modThisWeek, modLastWeek, modAllTime] = await Promise.all([
      summarizeMod(interaction.guildId, target.id, "this_week", weekStartDay),
      summarizeMod(interaction.guildId, target.id, "last_week", weekStartDay),
      summarizeMod(interaction.guildId, target.id, "all_time", weekStartDay),
    ]);
    ...
    const fields = [
      ...
      },
      {
        name: "Modstats",
        value:
          `**This week:** ${modThisWeek.total} (${modThisWeek.positive}+ / ${modThisWeek.negative}-)\n` +
          `**Last week:** ${modLastWeek.total} (${modLastWeek.positive}+ / ${modLastWeek.negative}-)\n` +
          `**All time:** ${modAllTime.total} (${modAllTime.positive}+ / ${modAllTime.negative}-)`,
        inline: false,
      },
    ];
