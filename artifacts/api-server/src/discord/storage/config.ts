...
export interface GuildChannels {
  ...
  banRequest?: string;
  /** Log channel for Anti-Nuke trigger events */
  antiNukeLog?: string;
  /** Log channel for Staff module actions */
  staffLog?: string;
  /** Log channel for Message Quota failures / results */
  quotaLog?: string;
  /** Log channel for Role Memory restore events */
...
export interface InfractionsConfig {
  /** Days until a strike expires. 0 = never. Default 30. */
  strikeExpiryDays: number;
  /** DM the staff member when an infraction is recorded. Default true. */
  dmOnInfraction: boolean;
  /** Auto-demote on 3+ active strikes. Default true. */
  autoDemotionEnabled: boolean;
  /** Action when a staff member accumulates 1 active strike. Default "warning". */
  strikeAction1: FailureAction;
  /** Action when a staff member accumulates 2 active strikes. Default "strike". */
  strikeAction2: FailureAction;
  /** Action when a staff member accumulates 3+ active strikes. Default "termination". */
  strikeAction3plus: FailureAction;
}

export interface QuotaFailureConfig {
  /** Action on 1st consecutive missed quota week. Default "warning". */
  failure1: FailureAction;
  /** Action on 2nd consecutive missed quota week. Default "strike". */
  failure2: FailureAction;
  /** Action on 3rd+ consecutive missed quota week. Default "termination". */
  failure3plus: FailureAction;
}

export interface ModerationConfig {
  /** DM users when a suspension action (ban/kick/mute/jail) is applied. Default true. */
  dmOnAction: boolean;
}

export interface PromotionsConfig {
  /** DM the promoted member. Default true. */
  dmMember: boolean;
}

export interface DemotionsConfig {
  /** DM the demoted member. Default true. */
  dmMember: boolean;
}

export interface AppealsConfig {
  /** Auto-close pending appeals after this many days. 0 = disabled. Default 0. */
  autoCloseDays: number;
}

export interface LoaConfig {
  /** Maximum LOA duration in days. 0 = unlimited. Default 0. */
  maxDurationDays: number;
  /** Require staff to supply a reason when requesting LOA. Default true. */
  requireReason: boolean;
}

export interface StaffReportConfig {
  /** How often to auto-refresh the staff report (hours). Default 2. */
...
export interface AntiJoinConfig extends AntiNukeMiniModuleConfig {
  /** How many join events within windowSeconds before punishing. Default 3. */
  threshold: number;
...
export interface AntiNukeConfig {
  enabled: boolean;
  /** Users who can open the anti-nuke config panel (beyond global whitelist + roles above bot). */
  accessUserIds: string[];
  /** Global whitelist applied to every mini-module. */
  globalWhitelistUserIds: string[];
  globalWhitelistRoleIds: string[];
  /** Punishment used when "Enable All" is triggered. */
...
export interface GuildConfig {
  ...
  /**
   * When true, any server member may use all whitelisted commands without
   * being individually added to a per-command or guild-all whitelist.
   * Set by `/verify-owner-commands`. Does not require a verified role or
   * the verify module to be enabled.
   */
  commandsUnlocked?: boolean;

  /**
   * Custom command prefix for this guild. Used for the `{prefix}n` DM command.
   * `bp?nuke` and `bp?highfi` are always `bp?` regardless of this setting.
   * Default: `b?`
   */
  guildPrefix?: string;
...
function withDefaults(c: Partial<GuildConfig> | undefined): GuildConfig {
  return {
    ...
    // Per-module settings — preserved as-is (getters provide defaults)
