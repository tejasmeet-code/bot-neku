...
export async function restoreBackup(guild: Guild, backup: ServerBackup): Promise<{ rolesCreated: number; channelsCreated: number; memberRolesRestored: number; errors: string[] }> {
  ...
  for (const br of backup.roles) {
    ...
    try {
      const created = await guild.roles.create({
        ...
        reason: `Zenvy restore from backup #${backup.id}`,
      });
  ...
  for (const bc of backup.channels.filter((c) => c.type === ChannelType.GuildCategory)) {
    ...
    try {
      const created = await guild.channels.create({ name: bc.name, type: ChannelType.GuildCategory, position: bc.position, reason: `Zenvy restore from backup #${backup.id}` });
  ...
  for (const bc of backup.channels.filter((c) => c.type !== ChannelType.GuildCategory)) {
    ...
    try {
      ...
      if (bc.type === ChannelType.GuildText || bc.type === ChannelType.GuildAnnouncement) {
        createdChannel = await guild.channels.create({ name: bc.name, type: bc.type as ChannelType.GuildText | ChannelType.GuildAnnouncement, parent: parentId, position: bc.position, topic: bc.topic, nsfw: bc.nsfw, rateLimitPerUser: bc.rateLimitPerUser, permissionOverwrites: permOverwrites, reason: `Zenvy restore from backup #${backup.id}` });
      } else if (bc.type === ChannelType.GuildVoice || bc.type === ChannelType.GuildStageVoice) {
        createdChannel = await guild.channels.create({ name: bc.name, type: bc.type as ChannelType.GuildVoice | ChannelType.GuildStageVoice, parent: parentId, position: bc.position, permissionOverwrites: permOverwrites, reason: `Zenvy restore from backup #${backup.id}` });
      } else if (bc.type === ChannelType.GuildForum) {
        createdChannel = await guild.channels.create({ name: bc.name, type: ChannelType.GuildForum, parent: parentId, position: bc.position, permissionOverwrites: permOverwrites, reason: `Zenvy restore from backup #${backup.id}` });
      }
      channelsCreated++;
  ...
  for (const [userId, roleIds] of memberMap) {
    ...
    for (const roleId of roleIds) {
      ...
      if (!member.roles.cache.has(liveRoleId)) {
        await member.roles.add(liveRoleId, `Zenvy restore from backup #${backup.id}`).catch(() => {});
        memberRolesRestored++;
      }
    }
  }
