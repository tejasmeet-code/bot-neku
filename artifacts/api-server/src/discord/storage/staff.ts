...
export async function recordDemotion(
  ...
  if (toRoleId === null) {
    // Termination: keep the in-memory snapshot we return so the caller can
    // build an embed, but purge the persisted profile so the user no longer
    // shows up in /staff-database, /staff-report, /staff-profile, etc.
...
/**
 * Hard-delete a staff profile. Used when someone is terminated/fired so they
 * disappear from every staff-database listing immediately. Returns true when
 * a profile actually existed and was removed.
 */
...
export async function recordInfraction(
  ...
  if (type === "termination") {
    // Mark and purge — same logic as a top-of-ladder demotion. The returned
    // infraction entry is preserved so the caller can still build the embed.
    profile.terminated = true;
    profile.terminatedAt = now;
    profile.currentRoleId = null;
    delete g.profiles[userId];
