...
export async function handlePrefixMessage(message: Message): Promise<void> {
  ...
  if (total > 1) {
    ...
    author
      .send(
        `${EMOJI_INFO} \`${PREFIX}\` started — sending to **${total}** members (${recipients.label}). ETA ~${formatSeconds(secs)}.`,
      )
      .catch(() => {});
  }
  ...
  author
    .send(
      `${EMOJI_INFO} \`${PREFIX}\` ran${where}. Sent to **${sent}** member${sent === 1 ? "" : "s"} (${recipients.label}).${failNote}`,
    )
    .catch((err) => {
