...
function buildClient(intents: GatewayIntentBits[]): Client {
  ...
  client.once(Events.ClientReady, (c) => {
    ...
    // Backfill on every joined guild: ensure Jailed role exists and register
    // slash commands per-guild so they're available instantly (no waiting on
    // global propagation, which can take up to ~1h on Discord's side).
    for (const guild of c.guilds.cache.values()) {
      ensureJailRole(guild).catch((err) => {
        logger.warn(
          { err, guildId: guild.id },
          "ensureJailRole failed on ready",
        );
      });
      registerGuildCommands(c, guild.id).catch((err) => {
        logger.warn(
          { err, guildId: guild.id },
          "registerGuildCommands failed on ready",
        );
      });
    }
    // Kick off the every-2h staff report auto-update loop, and run one
    // refresh now so any configured guild gets a freshly synced report
    // shortly after boot.
    startStaffReportAutoUpdate(c);
  ...
  client.on(Events.GuildCreate, async (guild) => {
    try {
      // Ensure the Jailed role exists and denies access to every channel.
      ensureJailRole(guild).catch((err) => {
        logger.warn({ err, guildId: guild.id }, "ensureJailRole failed on join");
      });
      // Register commands for this new guild immediately.
      registerGuildCommands(client, guild.id).catch((err) => {
        logger.warn(
          { err, guildId: guild.id },
          "registerGuildCommands failed on join",
        );
      });
...
async function registerGuildCommands(
  ...
  try {
    ...
    logger.info(
      ...
      "Slash commands registered for guild (instant availability)",
    );
  } catch (err) {
    logger.warn(
      { err, guildId },
      "Failed to register guild slash commands; falling back to global registration",
    );
  }
  void client;
}
...
export async function startDiscordBot(): Promise<void> {
  ...
  // Strategy: clear any leftover GLOBAL commands and rely on per-guild
  // registration that happens on ClientReady / GuildCreate. Per-guild
  // commands are available instantly (no ~1h propagation delay) — this
  // is what makes the bot work the moment it's online on Railway / Replit.
  try {
    logger.info(
      { count: visibleCommands.length, hidden: hiddenCount },
      "Clearing leftover global slash commands (will register per-guild instead)",
    );
    await rest.put(Routes.applicationCommands(clientId), { body: [] });
    logger.info("Global commands cleared; per-guild registration will run on Ready");
  } catch (err) {
    logger.error({ err }, "Failed to clear global slash commands");
  }
  ...
  const intentLevels: { intents: GatewayIntentBits[]; warn?: string }[] = [
    ...
    },
    {
      ...
      warn:
        "Message Content Intent is not enabled. The `bp?n` prefix command will not work until you enable it under your bot's 'Privileged Gateway Intents'. Continuing without it.",
    },
    {
      intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages],
      warn:
        "Server Members Intent is not enabled. /dm to roles or @everyone (and `bp?n` to roles/everyone) will not work until you enable it under your bot's 'Privileged Gateway Intents'. Continuing without it.",
    },
    {
      intents: [GatewayIntentBits.Guilds],
      warn:
        "All privileged intents are disabled. Slash commands work, but DM features and `bp?n` will not. Continuing in minimal mode.",
    },
  ];
