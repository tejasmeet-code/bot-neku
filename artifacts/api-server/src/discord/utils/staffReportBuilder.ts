...
const TIERS: { key: Tier; label: string; emoji: string }[] = [
  { key: "S", label: "Elite", emoji: "🏆" },
  { key: "A", label: "Excellent", emoji: "⭐" },
  { key: "B", label: "Reliable", emoji: "🔵" },
  { key: "C", label: "Average", emoji: "🟡" },
  { key: "D", label: "Underperforming", emoji: "🟠" },
  { key: "F", label: "At risk", emoji: "🔴" },
];

function formatLine(r: StaffRow): string {
  const tag = `<@${r.userId}>`;
  const role = r.positionLabel;
  const stats = `📈${r.promotions} 📉${r.demotions} ⚠️${r.warnings} 🔻${r.strikes}`;
...
/**
 * Build the staff tier report embed for a guild. Used by both the slash
 * command (`/staff-report`) and the periodic auto-update job.
 */
export async function buildStaffReportEmbed(
  ...
  const tiers: Record<Tier, StaffRow[]> = {
    S: [],
    A: [],
    B: [],
    C: [],
